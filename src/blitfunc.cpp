...
for (j = b->vblitsize; j--;) {
	for (i = hblitsize; i--;) {
		...
		if (pta) { bltadat = CHIPMEM_WGET_CUSTOM (pta); pta += 2; } else { bltadat = blt_info.bltadat; }
		bltadat &= blit_masktable_p[i];
		srca = (((uae_u32)preva << 16) | bltadat) >> b->blitashift;
		preva = bltadat;
		dstd = ((~srca & srcc));
		totald |= dstd;
		if (ptd) { CHIPMEM_WPUT_CUSTOM (ptd, dstd); ptd += 2; }
	}
...
for (j = b->vblitsize; j--;) {
	for (i = b->hblitsize; i--;) {
		...
		if (ptc) { srcc = CHIPMEM_WGET_CUSTOM (ptc); ptc += 2; }
		if (ptb) {
			uae_u32 bltbdat = CHIPMEM_WGET_CUSTOM (ptb); ptb += 2;
