...
for (j = b->vblitsize; j--;) {
	for (i = hblitsize; i--;) {
		...
		if (ptc) { srcc = CHIPMEM_WGET_CUSTOM (ptc); ptc += 2; }
		if (pta) { b->bltadat = bltadat = CHIPMEM_WGET_CUSTOM (pta); pta += 2; } else { bltadat = b->bltadat; }
		bltadat &= blit_masktable_p[i];
		srca = (((uae_u32)preva << 16) | bltadat) >> b->blitashift;
		preva = bltadat;
		if (dstp)
		  CHIPMEM_WPUT_CUSTOM (dstp, dstd);
		dstd = ((~srca & srcc));
		totald |= dstd;
...
for (j = b->vblitsize; j--;) {
	for (i = b->hblitsize; i--;) {
		...
		if (ptc) { srcc = CHIPMEM_WGET_CUSTOM (ptc); ptc += 2; }
		if (ptb) {
			uae_u32 bltbdat; b->bltbdat = bltbdat = CHIPMEM_WGET_CUSTOM (ptb); ptb += 2;
